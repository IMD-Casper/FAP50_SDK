...
libs:
	...
	@(cd $(DIR_SRC_LIB)/openjp2/ && $(CHMOD) -R +w * && $(MAKE)) || exit 1
	@(cd $(DIR_SRC_LIB)/openjp2/ && ($(CP) ./bin/libopenjp2.a $(EXPORTS_LIB_DIR))) || exit 1
...
config:
	@if [ "$(MSYS_FLAG)" = "-D__MSYS__" ]; then \
		...
		echo "Building openjp2 library for Android with NDK r23c..."; \
		(cd $(DIR_SRC_LIB)/openjp2 && mkdir -p build && cd build && \
		cmake -DCMAKE_TOOLCHAIN_FILE="$(ANDROID_NDK_HOME)/build/cmake/android.toolchain.cmake" \
		      ...
		      -D NBIS_ROOT="$(DIR_ROOT)" \
		      -DCMAKE_C_FLAGS="$(ARCH_FLAG) -O2 -w" \
		      .\
		) || exit 1; \
	fi;
#
install:
	...
	@(cd $(DIR_SRC_LIB)/openjp2 && $(CP) ./bin/libopenjp2.a $(INSTALL_ROOT_LIB_DIR)) || exit 1
	@(cd $(DIR_SRC_LIB)/openjp2 && $(CP) ./bin/libopenjp2.so $(INSTALL_ROOT_LIB_DIR)) || exit 1
#
clean:
	#@(cd $(DIR_SRC_LIB)/openjp2 && cd build && $(MAKE) clean && cd bin && $(RM) -rf *) || exit 1
	@(cd $(DIR_SRC_LIB)/openjp2 && $(RM) -rf build) || exit 1
#
catalog:
#
